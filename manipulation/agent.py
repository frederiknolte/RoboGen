...
class AgentIsaac:
    ...
    def init(self, body, id, np_random, indices=None):
        ...
        self.id = id
        self.np_random = np_random
        self.all_joint_indices = list(range(self.gym.get_asset_dof_count(body)))
        if indices != -1:
            pass
    ...
    def control(self, indices, target_angles, gains, forces):
        ...
        p.setJointMotorControlArray(self.body, jointIndices=indices, controlMode=p.POSITION_CONTROL,
                                    targetPositions=target_angles, positionGains=gains, forces=forces,
                                    physicsClientId=self.id)
    ...
    def get_joint_angles(self, indices=None):
        if indices is None:
            ...
            return []
        dof_states = self.gym.get_actor_dof_states(self.body, gymapi.STATE_NONE)
        return np.array([dof_states['pos'][i] for i in indices])
    ...
    def get_pos_orient(self, link):
        # Get the 3D position and orientation (4D quaternion) of a specific link on the body
        if link == self.base:
            transform = self.gym.get_rigid_transform(self.body)
        else:
            transform = self.gym.get_link_rigid_transform(self.body, link)
        pos = transform.p
        orient = transform.r
        return np.array(pos), np.array(orient)
    ...
    def get_velocity(self, link):
        if link == self.base:
            linear_velocity, angular_velocity = self.gym.get_rigid_body_velocity(self.body)
        else:
            linear_velocity, angular_velocity = self.gym.get_link_velocity(self.body, link)
        return np.array(linear_velocity)

    def get_euler(self, quaternion):
        return np.array(R.from_quat(quaternion).as_euler('xyz'))

    def get_quaternion(self, euler):
        return np.array(R.from_euler('xyz', euler).as_quat())

    def get_motor_joint_states(self, joints=None):
        # Get the position, velocity, and torque for nonfixed joint motors
        if joints is None:
            joints = self.all_joint_indices
        dof_states = self.gym.get_dof_states(self.body, gymapi.STATE_ALL)
        motor_states = [dof_states[i] for i in joints]
        motor_indices = [i for i in joints]
        motor_positions = [state['pos'] for state in motor_states]
        motor_velocities = [state['vel'] for state in motor_states]
        motor_torques = [state['effort'] for state in motor_states]
        return motor_indices, motor_positions, motor_velocities, motor_torques

    def get_joint_max_force(self, indices=None):
        if indices is None:
            indices = self.all_joint_indices
        dof_props = self.gym.get_dof_properties(self.body)
        return [dof_props['effort'][i] for i in indices]

    def set_base_pos_orient(self, pos, orient):
        orient if len(orient) == 4 else self.get_quaternion(orient)
        transform = gymapi.Transform()
        transform.p = gymapi.Vec3(*pos)
        transform.r = gymapi.Quat(*orient)
        self.gym.set_rigid_transform(self.body, transform)
    ...
    def enforce_joint_limits(self, indices=None):
        ...
        for j in indices:
            if joint_angles[j] < self.lower_limits[j]:
                p.resetJointState(self.body, jointIndex=j, targetValue=self.lower_limits[j], targetVelocity=0,
                                  physicsClientId=self.id)
            elif joint_angles[j] > self.upper_limits[j]:
                p.resetJointState(self.body, jointIndex=j, targetValue=self.upper_limits[j], targetVelocity=0,
                                  physicsClientId=self.id)
        ...
        if target_orient is not None:
            if use_current_as_rest:
                ik_joint_poses = np.array(p.calculateInverseKinematics(
                    self.body, target_joint,
                    targetPosition=target_pos, targetOrientation=target_orient,
                    lowerLimits=ik_lower_limits.tolist(), upperLimits=ik_upper_limits.tolist(),
                ...
                ik_joint_poses = np.array(
                    p.calculateInverseKinematics(self.body, target_joint, targetPosition=target_pos,
                                                 targetOrientation=target_orient, physicsClientId=self.id))
        else:
            if use_current_as_rest:
                ik_joint_poses = np.array(
                    p.calculateInverseKinematics(self.body, target_joint, targetPosition=target_pos,
                                                 restPoses=ik_rest_poses, maxNumIterations=max_iterations,
                                                 physicsClientId=self.id))
            else:
                ik_joint_poses = np.array(
                    p.calculateInverseKinematics(self.body, target_joint, targetPosition=target_pos,
                                                 maxNumIterations=max_iterations, physicsClientId=self.id))
    ...
    def set_joint_angles(self, indices, angles, use_limits=True, velocities=0):
        for i, (j, a) in enumerate(zip(indices, angles)):
            p.resetJointState(self.body, jointIndex=j,
